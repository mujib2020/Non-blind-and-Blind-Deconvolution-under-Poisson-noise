function [u,H,output] = FOTV_deconv_blind(f, pm)
...
for  it = 1:pm.maxit
    ...
    u1 = beta*c - pm.mu2*Z-lam2;
    u = -u1/(2*pm.mu2) + (sqrt(u1.^2 + 4*pm.mu2*beta*f1.*c))./(2*pm.mu2);
    
    
    %%%%%%%%%% v-subproblem%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
    [Dux,Duy]= D(u);
    Vx = Dux-lam_1x/pm.mu1;
    Vy = Duy-lam_1y/pm.mu1;
    W = sqrt (Vx.^2+Vy.^2);
    ...
    lam_1x = lam_1x + pm.mu1*(v1-Dux);
    lam_1y = lam_1y + pm.mu1*(v2-Duy);
    lam2 = lam2 + pm.mu2*(Z-u);
    ...
    xData = beta*sum(sum( Data_1- f.* log(abs(Data_1))));
