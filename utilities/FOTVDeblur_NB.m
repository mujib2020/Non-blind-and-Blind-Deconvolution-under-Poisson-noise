...
for i = 1:(K-1)
    w(K-i) = (-1)^i*gamma(pm.alpha+1)/gamma(pm.alpha-i+1)/factorial(i);
end
[D,Dt] = defDDt(w,pm.alpha); % compute fractional order derivatives
...
for i = 1:maxit
    ...
    %%%%%%%%%% u-subproblem%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % terms of numerator
    u1 = pm.mu2*real(ifft2(conj(A).*fft2(g)));
    ...
    u5 = pm.mu2 * AAt;
    uu = fft2(u1+u2+u3+u4)./(pm.mu1*C + u5);
    u = real(ifft2(uu));
    
    %%%%%%%%%%% z - subprblem%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [Dux,Duy]= D(u);
    Zx = Dux-lam_1x/pm.mu1;
    Zy = Duy-lam_1y/pm.mu1;
    
    W = sqrt (Zx.^2+Zy.^2);
    Max = max(W - 1/pm.mu1,0);
    W (W == 0) = 1;
    ...
    g2 = (pm.beta+lam2)/pm.mu2;
    g3 = (g1-g2)/2;
    
    g = g3 + sqrt (g3.^2 + pm.beta * f /pm.mu2);
    
    
    %%%%%%%%%%%%Lambda update%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    lam_1x = lam_1x + pm.mu1*(z1-Dux);
    lam_1y = lam_1y + pm.mu1*(z2-Duy);
    
    lam2 = lam2+ pm.mu2 * (g-g1); % q1 = Au
